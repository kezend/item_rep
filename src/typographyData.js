...
export async function loadFontFromSource(source, family, label = "Uploaded Font") {
...
export function buildFallbackTypographyData(text = "TEST") {
  ...
  const characters = [...String(text || "TEST")].filter((character) => !/\s/.test(character));
  const letterGuides = characters.map((_, index) => ({
    x: ((index + 0.5) / Math.max(characters.length, 1) - 0.5) * width,
    y: 0
  }));
...
export async function buildTypographyData(config) {
  ...
  lines.forEach((line, index) => {
    ...
    for (const character of line) {
      ...
      if (!/\s/.test(character)) {
        characterCenters.push({
          x: cursorX + glyphWidth * 0.5,
          y: baselineY
        });
      }
      cursorX += glyphWidth + scaledTracking;
    }
  });
  ...
  const outlineData = buildOutlineDataFromSegments(wirePositions);
  const letterGuides = characterCenters.map((center) => ({
    x: (center.x / sampleStep - (minColumn + maxColumn) / 2) * baseScale,
    y: ((minRow + maxRow) / 2 - center.y / sampleStep) * baseScale
  }));
